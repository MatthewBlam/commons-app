...
export function SearchPage({ visible }: SearchPageProps): React.JSX.Element {
  ...
  const hasMismatch = health && health.mismatchedChunks > 0 && health.totalChunks > 0 && !healthDismissed;
  ...
  return (
    <div className="min-h-full flex flex-col pt-3 pb-8">
      ...
      <div className="w-full max-w-3xl mx-auto px-10 flex-1 space-y-3">
        {hasMismatch && (
          <ErrorBanner variant="warning" onDismiss={() => setHealthDismissed(true)}>
            Some documents were embedded with a different model. Results may be less accurate. Re-sync your sources from the Sources tab to fix.
          </ErrorBanner>
        )}

        {rerankFailed && <ErrorBanner variant="warning">Reranking unavailable — results may be less accurate.</ErrorBanner>}
        ...
        {!loading && results !== null && results.length === 0 && <p className="text-sm text-muted-foreground text-center py-12">No results found. Try a different question or sync more docs.</p>}

        {!loading && results === null && !error && <EmptyState onSelectQuestion={handleSelectQuestion} />}
      </div>
    </div>
  );
}
