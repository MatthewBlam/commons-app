...
export function DrivePicker({ onSubmit, onCancel }: DrivePickerProps): React.JSX.Element {
  ...
  async function fetchFolder(folderId: string): Promise<void> {
    ...
    <div className="space-y-3">
      ...
      <div className="max-h-64 overflow-y-auto rounded-lg border border-input">
        {loading ? (
          ...
          <p className="px-3 py-6 text-center text-sm text-muted-foreground">This folder is empty.</p>
        ) : (
          <>
            {folders.map((folder) => (
              <div key={folder.id} className="flex items-center border-b border-input last:border-b-0">
                <div className="pl-3 pr-1 py-2">
                  <Checkbox checked={selected.has(folder.id)} onChange={() => toggleSelect(folder)} />
                </div>
                <button type="button" onClick={() => navigateInto(folder)} className="flex flex-1 items-center gap-2 py-2 pr-3 text-left text-sm hover:bg-accent/50 transition-colors min-w-0">
